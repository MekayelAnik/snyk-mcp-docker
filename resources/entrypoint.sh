...
readonly HAPROXY_SERVER_NAME="snyk-mcp"
...
start_mcp_server() {
    # Build the Snyk MCP command with configured options
    local snyk_mcp_cmd="snyk mcp -t stdio --disable-trust"

    # Add profile if specified
    local profile="${SNYK_MCP_PROFILE:-full}"
    case "$profile" in
        minimal|full|preview)
            snyk_mcp_cmd="$snyk_mcp_cmd --profile $profile"
            ;;
        *)
            echo "Invalid SNYK_MCP_PROFILE='${profile}', using default 'full'" >&2
            snyk_mcp_cmd="$snyk_mcp_cmd --profile full"
            ;;
    esac

    # Add debug flag if requested
    if is_true "${SNYK_DEBUG:-false}"; then
        snyk_mcp_cmd="$snyk_mcp_cmd -d"
    fi

    case "${PROTOCOL^^}" in
        SHTTP|STREAMABLEHTTP)
            CMD_ARGS=(npx --yes supergateway --port "$INTERNAL_PORT" --streamableHttpPath /mcp --outputTransport streamableHttp --healthEndpoint /healthz --stdio "$snyk_mcp_cmd")
            PROTOCOL_DISPLAY="SHTTP/streamableHttp"
            ;;
        SSE)
            CMD_ARGS=(npx --yes supergateway --port "$INTERNAL_PORT" --ssePath /sse --outputTransport sse --healthEndpoint /healthz --stdio "$snyk_mcp_cmd")
            PROTOCOL_DISPLAY="SSE/Server-Sent Events"
            ;;
        WS|WEBSOCKET)
            CMD_ARGS=(npx --yes supergateway --port "$INTERNAL_PORT" --messagePath /message --outputTransport ws --healthEndpoint /healthz --stdio "$snyk_mcp_cmd")
    ...
    echo "Launching Snyk MCP Server with protocol: ${PROTOCOL_DISPLAY}"
    echo "Snyk MCP profile: ${profile}"
...
main() {
    ...
    export SNYK_MCP_PROFILE="${SNYK_MCP_PROFILE:-full}"
    ...
    echo "=========================================="
    echo "Snyk MCP Server - Project Scan Directory"
    ...
    if [[ -d "$DATA_DIR" ]]; then
        ...
        if [[ "$project_count" -eq 0 ]]; then
            echo "  No projects found in ${DATA_DIR}"
            echo "  Mount project directories to ${DATA_DIR} for scanning"
        else
            echo "  Total projects available for scanning: ${project_count}"
        fi
    fi
    echo "=========================================="
